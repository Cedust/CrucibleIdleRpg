...
> Teil der [SPEC](../spec/README.md): Team, Stats, Attribute, Skilltree und Signatur-Skills.
> Verwandt: [Kampf](COMBAT-RUN.md) · [Items, Loot & Handwerk](ITEMS.md)
...
([BALANCING §2](../spec/BALANCE.md#1-wachstum-und-zahlenraum)); Attribut- und Crucible-Ebene sind
...
- **Offensive:** paarweise **Chance + Damage** je Muster (Crit, Multi Hit, Splash, Counter);
  Wirkung siehe [Charakter-Zug](DAMAGE-SYSTEM.md#11-charakter-zug-ausgehender-schaden). Die vier Paare
...
- Jeder Charakter hat einen Skilltree mit **vier Zweigen** — je ein Zweig pro offensivem
  ...
  aus der Rolle ([Zielauswahl](COMBAT-RUN.md#12-zielauswahl)) und den charaktergebundenen
...
- **Chance**-Stats haben einen **Soft-Cap bei 100 %** (Überschuss verpufft), **Damage**-Stats
  ...
  ([§2](#2-stats), [Charakter-Zug](DAMAGE-SYSTEM.md#11-charakter-zug-ausgehender-schaden)).
- Design-Absicht hinter der Zweig-Struktur, der Knoten-Multiplikation und der Platzierung der
  Crit-Erweiterungen: [DESIGN §3.2](../DESIGN.md#3-player-experience--der-kern-loop).
...
- Jeder Charakter trägt Ausrüstung in **sechs Slots**. Ausrüstung ist der **Hauptmotor** des
  Fortschritts ([BALANCING §3](../spec/BALANCE.md#1-wachstum-und-zahlenraum)).
