...
> Teil der [SPEC](../SPEC.md). Verbindliche Regeln für Rundenablauf, Zielauswahl,
> Kampfwerte und Formeln.
...
- Kampf ist **rundenbasiert** zwischen **eigenem Team** (drei Charaktere,
  [§3](CHARACTERS.md#3-team)) und einer **Gegnerformation** (bis zu sechs Gegner, §1.3).
- Der Kampf ist **deterministisch** und **render-unabhängig** simuliert (Simulation ≠ Rendering,
  inkrementelle Ausführung: [§5](SIMULATION.md#5-simulation--zeitverhalten-verbindlich)).
- Ergebnis eines Kampfes ist **Sieg** (alle Gegner besiegt) oder **Niederlage/Wipe**
  ...
  Fortschrittsweg, siehe [DESIGN §2](../DESIGN.md)).
...
- **Charaktere** haben einen festen Initiative-Wert (Stat,
  [§3](CHARACTERS.md#3-team)).
...
- Die Reihenfolge ist eine **totale Ordnung** in drei Stufen:
  ...
  3. bei Gleichstand innerhalb einer Seite **niedrigerer Slot-Index** zuerst — Charaktere in
     Team-Reihenfolge (Korvin → Rhaya → Quinn, [§3](CHARACTERS.md#3-team)), Gegner in
...
- **Suppression** ([§3.5](CHARACTERS.md#35-signatur-skills)) ist die einzige Operation, die
  die Queue umsortiert: Sie verschiebt die noch offene Aktion eines Gegners **innerhalb der
...
2. **Aktionen** in Initiative-Reihenfolge:
   ...
   - **Suppression** (Quinns Signatur-Skill, [§3.5](CHARACTERS.md#35-signatur-skills)) kann
     die noch offene Aktion eines Gegners **innerhalb der Runde nach hinten** verschieben —
     maximal bis an das Rundenende. Jeder lebende Gegner handelt weiterhin **genau einmal**
     pro Runde; eine Aktion entfällt ausschließlich, wenn der Gegner vor seinem verschobenen
     Zug **stirbt**.
...
- **Niederlage/Wipe:** alle Charaktere besiegt → Kampf endet ohne Belohnung (siehe
  [§4.4](PROGRESSION.md#44-checkpoints-wipe--abbruch)).
...
- **Kein Rundenlimit** — kein automatischer Runden-Cap.
  - **Endlichkeits-Zusicherung:** Gegner haben keine Heilung und Charakterangriffe verursachen
    stets vollen, positiven Schaden (§2.2), die Gegner-Gesamt-Health sinkt also **monoton** →
    jeder Kampf ist in **endlicher** Rundenzahl entschieden. Diese Zusicherung darf kein Feature
    verletzen (siehe [§4.6](RUNES.md#auslösung-verbindlich)).
  - **Kein Blockieren:** Es wird nur **eine Runde pro Anzeige-Takt** gerechnet, im Catch-up
    ebenfalls an Echtzeit gebunden ([§5](SIMULATION.md#5-simulation--zeitverhalten-verbindlich)).
...
- **Jeder erzeugte Treffer bemisst sich am rohen Grundschaden (vor Crit)** und würfelt
  **seinen eigenen** Crit-Wurf, sofern der zugehörige Skilltree-Knoten freigeschaltet ist
  ([§3.2](CHARACTERS.md#32-charakter-skilltree)). Crit wird dadurch pro Treffer **genau einmal**
  gezählt; es gibt keine Vererbung von Multiplikatoren zwischen Treffern.
...
1. **Basis-Verteilung.** `S` wird gleichmäßig auf die **lebenden** Charaktere verteilt:
   ...
   - **Mitigation** (Korvins Signatur-Skill,
     [§3.5](CHARACTERS.md#35-signatur-skills)) modifiziert diese
...
4. **Defense** (pro Charakter). **Flacher** Abzug (_Defense_ ist ein Derived Stat, gespeist u. a.
   aus _Toughness_, [§3.0](CHARACTERS.md#30-stats)) mit **prozentualer Untergrenze**:
   ...
   Runde handeln (Folgen für den Formations-Entwurf: [BALANCING §2](../BALANCING.md)).
...
- **Sunder** (Rhayas Signatur-Skill, [§3.5](CHARACTERS.md#35-signatur-skills)) baut den
  Bulwark-Beitrag einzelner Frontline-Gegner **während des Kampfes** ab (siehe dort).
...
- **Aller Zufall** in diesen Formeln (Treffer, Crit, Multi Hit, Splash, Counter,
  ...
  `Math.random()` ([AGENTS.md](../../AGENTS.md) §5, §14). Die **Ziehreihenfolge** ist Teil der
  Spezifikation (§2.1); Kampf, Gegner-Initiative und Loot laufen über **getrennte Ströme**
...
- Die Engine emittiert **Kampf-Events** (Crit, Multi Hit, Splash, Counter, Block, Rundenbeginn, …)
  ...
  ([§4.6](RUNES.md#46-runen-endgame--masterwork)) und die Grundlage des Playbacks
  ([§5](SIMULATION.md#5-simulation--zeitverhalten-verbindlich)) und unterliegen denselben
  Determinismus-Regeln wie der übrige Kampf.

### 2.6 Heilung — Grenzen und Auslösung

Heilquellen sind **Regeneration** (Stat, [§3.0](CHARACTERS.md#30-stats)) und im Endgame der
Rune-Effect `Heal` ([§4.6](RUNES.md#46-runen-endgame--masterwork)).
...
- **Barrier** ist ein Pool: Der Rune-Effect `Barrier` **addiert** auf den vorhandenen Rest.
  Der Reset zu Rundenbeginn (§1.1) erfasst ihn mit — Rune-Barrier verfällt also ebenso.
